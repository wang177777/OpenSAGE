...
- `data/supplementary/Supplementary_Data_2.xlsx`: Tables S11-S14, including analysis-set definitions and S12B raw domain summaries.
...
| `Reviewer-mean DGQC` | Mean of the 12 reviewer DGQC scores, reported as a sensitivity endpoint for Stage 2. |
...
Reviewer-level DGQC maps each 1-7 domain score to 0-100 and applies the Table S08 weights: `DGQC = Σ w_d × (D_d − 1) / 6`. Because this transformation is linear, applying it to the released mean domain profile reconstructs the corresponding reviewer-mean DGQC up to publication rounding.
